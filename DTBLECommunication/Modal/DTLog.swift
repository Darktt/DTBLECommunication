...
public func Delay(waitDuration duration: Double, execute: @escaping () -> Void)
{
    let delayInSeconds: Double = duration
    let delta: UInt64 = UInt64(delayInSeconds * Double(NSEC_PER_SEC))
    let popTime: DispatchTime = DispatchTime(uptimeNanoseconds: delta)
    DispatchQueue.main.asyncAfter(deadline: popTime, execute: execute)
}

public func Async(queue: DispatchQueue = DispatchQueue.main, execute: @escaping () -> Void)
{
    queue.async(execute: execute)
}

public func MainQueue(execute: @escaping () -> Void)
{
